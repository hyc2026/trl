...
class ValueModelTrainer(Trainer):
    ...
    def compute_loss(
        ...
        reward_logits = model.score(rewards_chosen.hidden_states[-1])
        reward_logits = reward_logits.squeeze(-1)[:, -1].squeeze(-1)
        
        if int(os.environ.get('LOCAL_RANK', 0)) == 0:
            print(reward_logits)
        loss = self.mse_loss(reward_logits, inputs["margin"])
